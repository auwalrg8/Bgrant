        ...
        <div className="text-center max-w-3xl">
          ...
          <div className="mb-8 relative">
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <svg className="w-48 h-48 animate-spin" style={{ animationDuration: '8s' }}>
                <circle
                  cx="96"
                  cy="96"
                  r="80"
          ...
          <h1 className="text-5xl md:text-6xl font-bold text-white mb-4">Unlock Bitcoin Dreams</h1>
          <p className="text-xl md:text-2xl text-orange-500 mb-4">Naija Builder? 🇳🇬</p>
          <p className="text-lg text-gray-400 mb-8">Grants in minutes • No KYC • zk-proofed</p>
          ...
          <div className="flex justify-center">
            <Button size="lg" onClick={() => setPhase('connect')} className="text-lg px-8 py-4">
              Connect Now
            </Button>
          </div>
          <p className="text-sm text-gray-500 mt-6 italic">Bukatar ku? (Hausa: "What do you need?")</p>
          {isReturning && (
            <div className="mt-8 p-4 bg-orange-500/10 border border-orange-500/30 rounded-lg">
              <p className="text-orange-500">Welcome back! New vouches available 🎉</p>
            </div>
          )}
        </div>
      </div>
            ...
            {phases.map((p, index) => (
              <Fragment key={p.id}>
                <div className="flex flex-col items-center">
                  ...
                  >
                    {p.completed ? <Check size={24} className="text-white" /> : index + 1}
                  </div>
                  <span className={`text-sm mt-2 ${phase === p.id ? 'text-white font-medium' : 'text-gray-400'}`}>
                    {p.label}
                  </span>
                </div>
                {index < phases.length - 1 && (
                  <div className={`flex-1 h-0.5 mx-2 ${p.completed ? 'bg-orange-500' : 'bg-gray-600'}`} />
                )}
              </Fragment>
            ))}
          </div>
        ...
        {phase === 'connect' && (
          <div className="max-w-2xl mx-auto">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-400">Choose your preferred connection method to get started</p>
            </div>
            <div className="space-y-4">
              ...
              <Card className="bg-gray-900 border-gray-600">
                <p className="text-sm text-gray-400">
                  ...
                  All connections are zk-sealed. Your data stays local until you choose to share.
                </p>
              </Card>
            </div>
          </div>
        )}
        {phase === 'scan' && <OSSScanProgress onComplete={handleScanComplete} />}
        {phase === 'preferences' && <PreferencesPanel onComplete={handlePreferencesComplete} />}
